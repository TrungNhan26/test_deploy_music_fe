...
async function getFullNameFromToken(token) {
  try {
    ...
    const response = await fetch(`http://27.71.20.51:8181/api/user/${encodedEmail}`, {
      method: 'GET',
      headers: {
...
async function fetchPurchasedMusic(token, username) {
  ...
  try {
    const url = `http://27.71.20.51:8181/api/user/musics/my-musics?composerUserName=${encodeURIComponent(
      username
    )}&page=1&pageSize=10`;
...
async function handleCreateMusic() {
  ...
  try {
    ...
    const springResponse = await fetch('http://27.71.20.51:8181/api/user/musics/create-music', {
      method: 'POST',
      headers: {
